...
export const ProgramCard: React.FC<ProgramCardProps> = ({
  ...
  const pricing =
    program.pricings?.find(
      (p) => (countryId ? p.countryId === countryId : false) || p.currency === currencyCode
    ) ||
  ...
  return (
    <div className="group relative flex flex-col justify-between rounded-2xl border border-glassBorder bg-white/90 p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-brand/40 hover:shadow-xl">
      <div>
        {/* Title */}
        ...
        <p className="mt-2 text-xs font-medium text-textMuted line-clamp-2">
          {program.description || "Industry-graded internship program with AI rubric evaluation."}
        </p>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-borderLight/60 pt-3.5">
        <div>
          ...
          <div className="text-[10px] font-medium text-textMuted">Inclusive of AI Evaluation</div>
        </div>

        <Link
          href={`/catalog/${program.slug}`}
          className="inline-flex items-center gap-1.5 rounded-xl bg-brand px-3.5 py-2 text-xs font-bold text-white shadow-sm hover:bg-brandHover transition-all cursor-pointer"
        >
          <span>Explore</span>
